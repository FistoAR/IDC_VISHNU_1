...
import Intarction from "./Intraction"
  ...
  return (
    <aside className="w-80 bg-white border-l border-gray-200 overflow-y-auto custom-scrollbar flex flex-col flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {/* ✅ GIF EDITOR (must come before image) */}
        {selectedElementType === "image" && isGif(selectedElement) && (
          <GifEditor selectedElement={selectedElement} onUpdate={onUpdate} />
        )}
        ...
        {!selectedElementType && (
          <div className="flex flex-col items-center justify-center h-64 text-center text-gray-400">
            ...
            <p className="text-xs mt-1 max-w-[200px]">
              Click on any text, image or video in the canvas to edit its
              properties.
            </p>
          </div>
        )}

        {/* INTERACTION PANEL for all selected elements */}
        {selectedElement && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <Intarction selectedElement={selectedElement} onUpdate={onUpdate} />
          </div>
        )}
      </div>
    </aside>
