  ...
  useEffect(() => {
    if (iframeRef.current) {
      ...
      setTimeout(() => {
        ...
        // Listen for internal crop requests (from double click)
        const handleCropRequest = (e) => {
          if (onElementSelect && e.detail?.src) {
            // Re-select and signal crop mode
            onElementSelect(null, 'image', { openCrop: true, src: e.detail.src });
          }
        };
        doc.addEventListener('open-crop-tool', handleCropRequest);

        return () => {
          doc.removeEventListener('open-crop-tool', handleCropRequest);
        };
      }, 100);
    }
  }, [templateHTML, onElementSelect]);
  ...
  const setupEditableElements = (doc) => {
    ...
    images.forEach(img => {
      ...
      const handleImageDblClick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        // Trigger a custom event that parent can listen to, or we can assume parent's ImageEditor will handle it if we signal
        const cropEvent = new CustomEvent('open-crop-tool', { detail: { src: img.src } });
        doc.dispatchEvent(cropEvent);
      };
