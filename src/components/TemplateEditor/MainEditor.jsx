...
const MainEditor = () => {
  ...
  const lastSelectedMetaRef = useRef(null);
  ...
  const handleUndo = useCallback(() => {
    ...
    if (previousState) {
      console.log("MainEditor: Performing Undo");
      ...
      // Attempt to re-select after iframe reload
      attemptReSelection();
    }
  }, [undo]);
  ...
  const handleRedo = useCallback(() => {
    ...
    if (nextState) {
      console.log("MainEditor: Performing Redo");
  ...
  const attemptReSelection = () => {
    if (!lastSelectedMetaRef.current) return;

    // Wait for iframe reload and setupEditableElements
    setTimeout(() => {
      const iframe = document.querySelector('iframe[title="Template Editor"]');
      if (iframe && iframe.contentDocument) {
        const doc = iframe.contentDocument;
        const meta = lastSelectedMetaRef.current;

        // Find element by stored index or unique properties
        const elements = doc.querySelectorAll(meta.tagName);
        const element = Array.from(elements).find((el, idx) => {
          // We can't rely on index if elements were added/removed
          // But for style changes, index usually works
          return idx === meta.index;
        });

        if (element) {
          console.log("MainEditor: Re-selecting element", meta.tagName);
          element.click(); // Trigger handleElementSelect through the DOM listener
        }
      }
    }, 350);
  };
  ...
  const handleElementSelect = useCallback((element, type, options = {}) => {
    setSelectedElement(element);
    setSelectedElementType(type);

    if (element) {
      // Track metadata for re-selection
      const tagName = element.tagName.toLowerCase();
      const allSameTag = Array.from(element.ownerDocument.querySelectorAll(tagName));
      const index = allSameTag.indexOf(element);

      lastSelectedMetaRef.current = { tagName, index };
    } else if (!options?.isInternalSelection) {
