...
/**
 * Canva-like editor foundation (React + Vite + Tailwind + Fabric.js)
 * - Reliable editable text (IText) + text toolbar
 * - Layers panel (select/hide/delete)
 * - Import SVG (grouped to keep alignment) + ungroup
 * - Import PSD (layers as images via ag-psd)
 * - Save/Load project JSON
 * - Export PNG + SVG
 * - Zoom + Pan
 *
 * Notes:
 * - SVG <text> rarely imports as truly editable text. Use editor text tool instead.
 * - PSD text usually rasterizes (pixels).
 */
...
export default function App() {
  ...
  const ungroup = (id) => {
    ...
    if (sel?.type === "activeSelection") {
      sel.getObjects().forEach((child, idx) => {
        if (!child.id) child.id = makeId();
        if (!child.name) child.name = `SVG Item ${idx + 1}`;
      });
    }
    canvas.requestRenderAll();
    setLayers(canvas.getObjects().slice().reverse());
    ...
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <div className="w-80 bg-gray-800 border-l border-gray-700 p-4 overflow-y-auto">
        ...
        <div className="mt-4 text-xs text-gray-400 leading-5">
          <div className="font-semibold mb-1">Why SVG text looks wrong?</div>
          <div>
            Many SVGs use fonts you don’t have, or convert text to outlines/paths. Fabric can’t always
            rebuild editable text. For Canva-like behavior, create text inside the editor.
          </div>
        </div>
      </div>
    </div>
